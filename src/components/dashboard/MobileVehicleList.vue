...
const updateVehicleType = async () => {
  ...
  try {
    const response = await axios.put(
      `editmobileadslicensebyadmin/${props.appId}`,
      vehicleTypeData
    );
...
const addNewLicense = async (payload) => {
  ...
  try {
    ...
    const response = await axios.put(
      `addmobileadsbyadmin/${props.appId}`,
      data
    );
    console.log(response);
    if (response.status === 200) {
...
const deleteLicense = async (licenseId) => {
  try {
    const response = await axios.delete(
      `deletelicencebyadmin/${props.appId}/${licenseId}`
    );
...
<template>
  <li class="mb-10">
    ...
    <button
      ...
      v-if="!editVehicleLicense && !status && editable"
    >
      Edit Licenses
    </button>
  </li>
