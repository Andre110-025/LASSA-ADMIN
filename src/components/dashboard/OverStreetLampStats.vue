...
const getStats = async () => {
  try {
    ...
    const { data } = await axios.get(
      `streetlampreport?start_date=${props.date.start_date}&end_date=${props.date.end_date}`
    );
    // console.log(response.data);
    // console.log(props.date);

    stats[0].amount = data.total_street_lamp_ads;
    ...
    pieData.type_of_sign = {
      labels: Object.keys(data.type_of_sign),
      datasets: [
        {
          label: "Type of Sign",
          data: Object.values(data.type_of_sign),
          backgroundColor: getRandomBrightColors(
            Object.keys(data.type_of_sign).length
          ),
          cutout: "80%",
        },
      ],
    ...
    pieData.total_type_of_sign = Object.values(data.type_of_sign).reduce(
      (sum, val) => sum + val,
      0
    );
    loading.value = false;
...
<template>
  ...
  <div v-if="!loading" class="flex md:flex-row flex-col my-5 gap-4">
    <PieChart
      :statsHeading="'Type of Sign'"
      :DoughnutData="pieData.type_of_sign"
      :appTotal="pieData.total_type_of_sign"
    />
  </div>
</template>
