...
const getRules = async () => {
  try {
    ...
    const { data, status } = await axios.get(`getmobileadsapplicationpricing`);
...
const saveEdit = async () => {
  try {
    ...
    // Validate form - only validate app_price
    if (!editForm.value.app_price) {
      toast.error("Please fill in the app price");
      return;
    }
    ...
    if (isNaN(appPrice)) {
      toast.error("Please enter a valid numeric value for app price");
      return;
    }

    if (appPrice < 0) {
      toast.error("App price cannot be negative");
      return;
    }

    // Prepare payload to include all fields
    const payload = {
      app_price: appPrice,
      min_number: editForm.value.min_number,
      max_number: editForm.value.max_number,
    };

    // Make API call to update rule
    const { data, status } = await axios.post(
      `updatemobileadsapplicationpricing/${editForm.value.id}`,
      payload
    );
...
const formatRange = (minNumber, maxNumber) => {
  if (minNumber === null && maxNumber === null) return "All";
  if (minNumber === null) return `Up to ${maxNumber}`;
  if (maxNumber === null) return `${minNumber} and above`;
        ...
        <tr v-for="rule in rules" :key="rule.id" class="hover:bg-gray-50">
          ...
          <template v-if="editingRule === rule.id">
            ...
            <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
              {{ formatRange(rule.min_number, rule.max_number) }}
            </td>
            <td class="px-4 py-4">
              <input
