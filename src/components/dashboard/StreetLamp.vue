...
const rules = computed(() => ({
  user_id: { required },
  name_of_display: { required },
  purpose: { required },
  type_of_sign: { required },
  start_date: { required },
  duration: { required },
  source_of_electricity: {
    ifLed: requiredIf(streetData.type_of_sign === "LED"),
  },
  location_of_generator: {
    ifLed: requiredIf(streetData.source_of_electricity === "Generator"),
  },
  authorization_letter_lseb: {
    ifLed: requiredIf(
      streetData.source_of_electricity === "Public/Private Electricity Supply"
    ),
  },
  photo_montage: { required },
  streetlampform: {
    streetlampinfo: {
      $each: helpers.forEach({
        number_poles: { required },
        site_address: { required },
        owner_consent: { required },
      }),
    },
  },
}));

const v$ = useVuelidate(rules, streetData);
...
const submitStreetData = async () => {
  loading.value = true;
  try {
    const response = await axios.post("createstreetlampforuser", streetData);
