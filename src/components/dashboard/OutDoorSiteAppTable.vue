...
const getEndPoint = () => {
  if (props.task) {
    return "getthirdpartyoutdoorsignagetask";
  } else {
    return "getalloutdoorapplication";
  }
};
    ...
    <table class="w-full mt-6">
      <thead class="border-b-[1px] border-b-gray-900 bg-slate-50">
        <tr class="*:whitespace-nowrap">
          <th
            class="xs:text-sm text-xs font-medium text-gray-700 text-left p-2.5 hidden md:table-cell"
            scope="col"
          >
            User Identifier
          </th>
          ...
          <th
            class="xs:text-sm text-xs font-medium text-gray-700 text-left p-2.5 table-cell"
            scope="col"
          >
            Details
          </th>
      ...
      <tbody>
        <tr
          v-for="(app, index) in applicationInfo.data"
          :key="index"
        ...
        >
          ...
          <td
            class="p-2.5 text-xs xs:text-sm hidden md:table-cell"
            v-text="formatDate(app.updated_at)"
          ></td>
          <td class="p-2.5 table-cell text-xs xs:text-sm">
            {{ app.type_sign }}
          </td>
          <td class="p-2.5 hidden lg:table-cell text-xs xs:text-sm">
            {{ app.ongoing_request || 'None' }}
          </td>
          <td class="p-2.5 text-xs xs:text-sm">
            <a
              :href="`requests/out-door-site-detail/${app.id}`"
              target="_blank"
