...
const headerTitles = ["User Identifier", "Payment ID", "Status", "Location", "App Fee", "Actions"];

const applicationInfo = ref({});

const getEndPoint = () => {
  if (props.task) {
    return "gettemporaryadstask";
  } else {
    return "admingetallfirstpartytemporrayads";
  }
};
      ...
      <thead class="border-b-[.0938rem] border-gray-900 bg-slate-50">
        <tr>
          <th
            ...
            :key="index"
            class="text-sm text-gray-700 text-left p-2.5 *:whitespace-nowrap"
            :class="{
              'hidden sm:table-cell': header === 'App Fee',
      ...
      <tbody>
        ...
        >
          ...
          <td class="p-2.5 hidden md:table-cell text-xs xs:text-sm">
            <IconAppStatus :status="app.application_status" />
          </td>
          <td
            class="p-2.5 text-xs xs:text-sm capitalize hidden md:table-cell"
            v-text="app.site_location"
          ></td>
          ...
          <td class="p-2.5 text-xs xs:text-sm">
            <a
              target="_blank"
              :href="`requests/temporary-app-detail/${app.id}`"
