...
async function getData(page = 1) {
  loading.value = true;
  const apiUrl = props.task ? "getonpremisetask" : "getfirstpartyonpremise";
  ...
  <div class="w-full overflow-x-auto" v-if="applicationInfo.data?.length && !loading">
    <table class="w-full mt-6">
      <thead class="border-b-[1px] border-b-gray-900 bg-slate-50">
        <tr>
          <th
            class="p-2.5 text-xs xs:text-sm hidden whitespace-nowrap lg:table-cell font-medium text-gray-700"
            scope="col"
            >User Identifier</th>
          <th
            v-for="(header, index) in headerTitles"
            :key="index"
            class="xs:text-sm text-xs font-medium whitespace-nowrap text-gray-700 lg:w-[14.5%] w-1/3 text-left p-2.5"
            scope="col"
            :class="{
              'hidden lg:table-cell': ['Purpose', 'Application Date'].includes(
                header
              ),
