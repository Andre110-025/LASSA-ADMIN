...
const addNewMontage = async () => {
  ...
  try {
    ...
    const response = await axios.put(
      `updatemontagemobilebyadmin/${props.appId}`,
      imgList
    );
...
<template>
  ...
  <form @submit.prevent="addNewMontage()" v-if="!status && editable">
    <div class="inputHolder">
      <label for="multipleMontage" v-show="!names.length">
        ...
        >
          ...
          <span class="text-sm mt-2.5"> Click to Select Images <br> (Each Image should not be more than 1MB) </span>
        </div>
      </label>
      <input
        id="multipleMontage"
