...
const filteredStreetCategory = computed(() => {
  if (props.signData.street_category.includes('LED')) {
    return streetCategory.filter(category => category !== 'Restrictive Zone N');
  } else {
    return streetCategory.filter(category => category !== 'Encroachment Zone N');
  }
});
...
const billCategoryData = reactive({
  ...
  ads_price_pyears: props.signData?.ads_price_pyears || 0,
});

const generateBill = async () => {
  if (!billCategoryData.streetClassification) {
    return;
  }
  ...
  try {
    ...
    const { data } = await axios.post(
      `billingoutdoorgeneration/${props.id}`,
      billCategoryData
    );
...
<template>
  ...
  >
    <div class="flex flex-col">
      ...
      <div v-if="!adType">
        ...
        >
          <div class="inputHolder">
            ...
            >
              <option
                v-for="(item, index) in streetCategory"
                :key="index"
                :value="item"
                v-text="item"
              ></option>
          ...
          >
            <input
              type="number"
              :min="billCategoryData.ads_price_pyears ? 5000 : 0"
              placeholder=""
              class="input peer"
              id="price_rate"
              autocomplete="off"
              v-model="billCategoryData.ads_price_pyears"
              step=".01"
              required
            />
            <label for="price_rate" class="inputLabel">Past Year Bill</label>
          </div>
          ...
          >
            <input
              ...
              required
            />
            <label for="price_rate" class="inputLabel">Advert Rate</label>
          </div>
        ...
        <div v-else class="py-2.5 px-10 space-y-5">
          <div class="flex gap-4">
            ...
            <p
              class="grow"
              v-text="formatCurrency(billData['signage bill'])"
            ></p>
          </div>
          <div class="flex gap-4">
            <p class="font-medium grow">Legal Fee</p>
            <p class="grow" v-text="formatCurrency(billData['legal fee'])"></p>
          </div>
      ...
      <div class="min-h-[280px] p-5 flex flex-col justify-between" v-else>
        <p class="text-center my-5 font-semibold">Generated Bill</p>

        <div class="flex gap-4">
          <p class="font-medium grow">Signage Bill <br> <i v-if="userDetails.userInfo.department === 'Registry'" class="text-xs">(1% legal fee inclusive)</i></p>
          <p class="grow" v-text="formatCurrency(temporalBill)"></p>
        </div>
        <div class="flex items-center">
          <button
            type="button"
